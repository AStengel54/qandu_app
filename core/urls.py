...
urlpatterns= patterns ('',
   ...
   url(r'^user/update/(?P<slug>\w+)/$', login_required(UserUpdateView.as_view()), name='user_update'),              url(r'^user/delete/(?P<slug>\w+)/$', login_required(UserDeleteView.as_view()), name='user_delete'),
   url(r'^seach/$', login_required(SearchQuestionListView.as_view()), name= 'search'),
)